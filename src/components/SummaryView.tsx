import Image from "next/image";

function BlueskyIcon({ link, size }: { link?: string; size?: string }) {
  return (
    <a href={link || "https://bsky.social"} target="_blank">
      <svg
        viewBox="0 0 360 320"
        xmlns="http://www.w3.org/2000/svg"
        className={`fill-current text-primary-500 dark:text-primary-400 ${size || "h-6 w-6"}`}
      >
        <path d="M180 141.964C163.699 110.262 119.308 51.1817 78.0347 22.044C38.4971 -5.86834 23.414 -1.03207 13.526 3.43594C2.08093 8.60755 0 26.1785 0 36.5164C0 46.8542 5.66748 121.272 9.36416 133.694C21.5786 174.738 65.0603 188.607 105.104 184.156C107.151 183.852 109.227 183.572 111.329 183.312C109.267 183.642 107.19 183.924 105.104 184.156C46.4204 192.847 -5.69621 214.233 62.6582 290.33C137.848 368.18 165.705 273.637 180 225.702C194.295 273.637 210.76 364.771 295.995 290.33C360 225.702 313.58 192.85 254.896 184.158C252.81 183.926 250.733 183.645 248.671 183.315C250.773 183.574 252.849 183.855 254.896 184.158C294.94 188.61 338.421 174.74 350.636 133.697C354.333 121.275 360 46.8568 360 36.519C360 26.1811 357.919 8.61012 346.474 3.43851C336.586 -1.02949 321.503 -5.86576 281.965 22.0466C240.692 51.1843 196.301 110.262 180 141.964Z"></path>
      </svg>
    </a>
  );
}

export const SummaryView = () => {
  return (
    <div className="flex flex-row md:flex-col gap-4">
      <Image
        src="/images/profile_square.jpg"
        width={208}
        height={208}
        className="max-h-52 md:hidden"
        alt="photo of me"
      />
      <Image
        src="/images/profile_tall.jpg"
        width={208}
        height={312}
        className="hidden md:block"
        alt="photo of me"
      />
      <div className="flex flex-col gap-2">
        <div>
          <p className="font-light font-md">Geoff Cramer</p>
          <div className="font-light inline-flex items-center gap-2">
            <p>Software Engineer @</p>
            <BlueskyIcon size="h-[1.2em] w-[1.2em]" />
          </div>
          <p className="font-light">gcwill70@gmail.com</p>
        </div>
        <div className="flex flex-row gap-3">
          <BlueskyIcon
            link="https://bsky.app/profile/jeffwithag.net"
            size="h-10 w-10"
          />
          <a href="https://www.linkedin.com/in/geoff-cramer/" target="_blank">
            <Image
              src="/images/ic_linked_in.svg"
              alt="LinkedIn"
              width={40}
              height={40}
              className="h-10 w-10"
            />
          </a>
          <a href="https://github.com/gcwill70" target="_blank">
            <Image
              src="/images/ic_github.png"
              alt="GitHub"
              width={40}
              height={40}
            />
          </a>
        </div>
      </div>
    </div>
  );
};
